...
class VolzServo
{
  public:
    ...
    void setSPos(byte servo, short pos); //set absolute servo position. Valid range is +- 1934.
    ...
    void sendCmd(unsigned char * ans, unsigned char * cmd, short timeout = 6, bool suppressOutput = false); // Send a command to the RS485 bus. Returns the response via ans pointer.
